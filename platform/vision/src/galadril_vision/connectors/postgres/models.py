...
class EntityState(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_entity_states_geom", "geom", postgres_using="gist"),
        Index(
...
class CausalRun(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_causal_runs_target", "target", text("created_at DESC")),
    )
...
class AuthzOutbox(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "tenant_id <> ''", name="ck_authz_outbox_tenant_nonempty"
        ),
        CheckConstraint(
            "object_id <> ''", name="ck_authz_outbox_object_nonempty"
        ),
        CheckConstraint(
        ...
        Index("idx_authz_outbox_retry", text("next_retry_at ASC")),
    )
