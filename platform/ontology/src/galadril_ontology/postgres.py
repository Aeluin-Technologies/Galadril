...
ON ontology_revision_parents (tenant_id, parent_revision_id, revision_id);

REVOKE INSERT, UPDATE, DELETE ON ontology_base_artifacts FROM galadril_app;
GRANT SELECT ON ontology_base_artifacts TO galadril_app;
...
BEGIN
    FOREACH protected_table IN ARRAY ARRAY[
        ...
        EXECUTE format('REVOKE ALL ON public.%I FROM PUBLIC', protected_table);
    END LOOP;
END
$rls$;
"""
...
class PostgresOntologyRepository:
    ...
    async def initialize_schema(self) -> None:
        """Installs tables, invariants, RLS, and immutable-history triggers."""
        async with self._connections.maintenance_connection() as connection:
            await connection.execute(ONTOLOGY_SCHEMA_SQL)
